...
for (i=0; i<fileList.length; i++) {
	...
	for(j=0; j<nStacks; j++) {	
		...
		if(currentChannelCount == 0 && manualSettings){
			setBatchMode(false);
		}
		else{
			setBatchMode(true);
		...
		if(!matches(seriesTitle, ".*/.*/.*$") && matches(seriesTitle, ".*Series[0-9][0-9][0-9]$")){
			...
			//Get the number of channels in the current stack
			getDimensions(dummy, dummy, stackChannels, dummy, dummy);

			//Once the first run is complete turn on batch mode
			if(currentChannelCount != stackChannels && manualSettings){
				setBatchMode(false);
			}
			else{
				setBatchMode(true);
			...
			if(currentChannelCount != stackChannels){
				for(a=0; a<stackChannels; a++){
					...
					currentChannelCount = stackChannels;
					print("New lowest channel is " + lowestChannel+1 + " of " + currentChannelCount+1 + " total channels.");
				}
			...
			for(a=0; a<currentChannelCount; a++){
				currentTitle = replace(imageTitle, "C=.", "C=" + a);
				selectWindow(currentTitle);
				if(a != lowestChannel){
					close();
